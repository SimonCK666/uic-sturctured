...
void readLeson(double *num11,char *ch11,double *num21,char *ch21,double *num31,char *ch31)
{
	char name[10];
	int n;
	Course math;
	...
	//}
	while (fscanf(fp, "%s %d %lf %c",name,&n,&num11[i],&ch11[i] ) != EOF)
	{
		//printf("%0.2f %c\n", num1[i], ch1[i]);
		i++;
	}
	...
	i = 0;
	while (fscanf(fp, "%s %d %lf %c", name, &n, &num21[i], &ch21[i]) != EOF)
	{
		//printf("%0.2f %c\n", num21[i], ch21[i]);
		i++;
	}
	...
	fp = fopen("C.txt", "r");
	i = 0;
	while (fscanf(fp, "%s %d %lf %c", name, &n, &num31[i], &ch31[i]) != EOF)
	{
		//printf("%0.2f %c\n", num3[i], ch3[i]);
		i++;
	}
...
void import_Course(Student* st, double* num11, char* ch11, double* num21, char* ch21, double* num31, char* ch31)
{
	/*for (int i = 0; i < 7; i++)
	{
		printf("%c %f\n", ch21[i], num21[i]);
