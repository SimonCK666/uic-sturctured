...
typedef struct n
{
	// Linked list to store the course name and score
	char courseName[20];
	double score;
	// Grade (a,b,c,d)
	char Sn[2];
...
typedef struct st
{
	// nested struct to make the linked list in the student struct
	Data *data;
	char name[10];
	int ID;
	double Gpa;
...
void delet_Course(Student* st)
{
	int i;
	char course_name[10];
	printf("Please enter the course name: "); scanf("%s", course_name);
...
It's used to determine whether the corresponding person is going to output, if there's no isok=1 otherwise isok=0*/
	...
	for (i = 0; i < 7; i++)
	{
		for (j = 0; j < 7; j++)
		{
			if (id_list[i] == st[j].Gpa&&isok[j]==1)
			{
				Data* p = st[j].data->next;
				printf("\n%s\t%d\t", st[j].name, st[j].ID);
				while (p)
...
//}
	...
	while (1)
	{
		...
		getchar();
		printf("Please press enter Enter to continue. . .\n");
		getchar();
		system("CLS");
	}
