...
void import_Course(Student* st)
{
	...
	int i, j;
	char course[20];
	printf("Please input the course name(length of course name is less than 20.):\n");
	scanf("%s", course);
	if (strlen(course) < 20)
	{
		for (i = 0; i < 6; i++)
		{
			...
			printf("%s的分数：", st[i].name); scanf("%lf", &data->score);
			getchar();
			printf("%s的等级：", st[i].name); scanf("%c", &data->Sn);
