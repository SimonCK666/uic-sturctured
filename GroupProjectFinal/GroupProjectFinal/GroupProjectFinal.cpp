...
void import_Course(Student* st)
{
	...
	if (strlen(course) < 20)
	{
		for (i = 0; i < 7; i++)
		{
			...
			while (p!=NULL)
			{
				// Determine if the course will be replaced
	...
	{
		// Determine course titles over 20
...
void delet_Course(Student* st)
{
	...
	// Delete the course through the linked list
	for ( i = 0; i < 7; i++)
	{
		Data* p = st[0].data->next;
		Data* q = st[0].data;
...
void Sorted_Id(Student *st)
{
	...
	// output the ordered information
	for ( i = 0; i < 7; i++)
	{
		for ( j = 0; j < 7; j++)
		{
